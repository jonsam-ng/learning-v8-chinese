...
这个项目的唯一目的是帮我学习谷歌的 V8 JavaScript 引擎。
...
Isolate是V8运行时的一个独立副本，包括它自己的堆。两个不同的Isolate可以并行运行，可以被看作是完全不同的V8运行时的沙盒化实例。

## Context（上下文）

为了允许不同的JavaScript应用程序在同一个隔离区（isolate）中运行，必须为每个应用程序指定一个上下文。 这是为了避免它们互相干扰，例如，改变所提供的内置对象。

## Template（模板）

这是ObjectTemplate和FunctionTemplate的父类（super class）。记住，在JavaScript中，函数可以像对象一样拥有字段（fields）。
...
`Set`函数可以用来在由该template创建的实例上设置一个name和一个value。`SetAccessorProperty`是用于使用functions时 get/set属性。
...
which is from clang's c++ library which does not use namespaces (__11 or__14 etc).
